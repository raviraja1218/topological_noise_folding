...
def train_epoch(model, loader, loss_fn, optimizer, epoch):
    ...
    for batch in loader:
        ...
        # Prepare graphs list for symbolic penalty
        graphs = [batch]  # Pass the batch as a list of graphs
        
        # Compute loss
        loss, log_dict = loss_fn(
            tcount_pred, gamma_pred, tcount_true, gamma_true,
            graphs, epoch=epoch, training=True
        )
...
def validate(model, loader, loss_fn, epoch):
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            loss, log_dict = loss_fn(
                tcount_pred, gamma_pred, tcount_true, gamma_true,
                graphs, epoch=epoch, training=False
            )
...
def main():
    print("=" * 60)
    print("PHASE 3: Graph Transformer Training (Fixed v2)")
    print("=" * 60)
    print(f"\nConfiguration:")
