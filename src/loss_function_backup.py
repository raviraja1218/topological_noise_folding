...
class NeuroSymbolicLoss(nn.Module):
    ...
    def forward(self, tcount_pred, gamma_pred, tcount_true, gamma_true, 
        ...
        # Symbolic penalty
        loss_symbolic = self.compute_symbolic_penalty(graphs)
    ...
    def compute_symbolic_penalty(self, graphs):
            ...
            if hasattr(graph, 'M') and hasattr(graph, 'N') and hasattr(graph, 'C'):
                ...
                try:
                    ...
                    if torch.abs(trace_exp - n) < 1e-5:
                        # DAG - no penalty
                        penalty = torch.tensor(0.0, device=NC.device)
                    else:
                        # Cycles detected
                        violation_count = torch.sum(torch.abs(exp_NC - torch.eye(n, device=NC.device)) > 1e-5)
                    ...
                    if cycles_detected:
                        penalty = torch.tensor(2.0, device=NC.device)
                    else:
                        penalty = torch.tensor(0.0, device=NC.device)
            else:
                # No flow matrices - assume invalid
                penalty = torch.tensor(10.0, device=tcount_pred.device)
