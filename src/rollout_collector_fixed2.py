...
class ReplayBuffer:
    ...
    def sample(self, batch_size):
        ...
        if len(self.buffer) < batch_size:
            # Not enough samples - return random indices with replacement
            if len(self.buffer) == 0:
                # Buffer empty - return empty list
                return []
...
class DistributedTrainer:
    ...
    def __init__(self, agent, env_creator, num_workers=2, buffer_size=1000):
        self.agent = agent
        self.env_creator = env_creator
        self.num_workers = num_workers
    ...
    def train_step(self, steps_per_update=100):
        ...
        for rollout in rollouts:
            if rollout:  # Only add non-empty rollouts
                self.buffer.push(rollout)
            
        # Sample batch
        batch = self.buffer.sample(batch_size=16)  # Smaller batch size for testing
        
        # Update agent (only if we have samples)
        if batch:
        ...
        return {
            ...
            'rollouts_collected': len([r for r in rollouts if r])
        }
...
if __name__ == "__main__":
    ...
    # Create trainer
    trainer = DistributedTrainer(agent, env_creator, num_workers=2, buffer_size=100)
    
    # Test training step
    for i in range(3):
        stats = trainer.train_step(steps_per_update=20)
        print(f"Step {i+1}: loss={stats['loss']:.4f}, buffer={stats['buffer_size']}, rollouts={stats['rollouts_collected']}")
