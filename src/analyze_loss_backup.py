...
# 2. Loss Landscape (2D contour)
print("\n🗺️  Generating loss landscape...")

# Create grid for lambda values
...
for l in lambdas:
    # Simulate trade-off
    task = 500 * (1 + 0.1 * np.random.randn())
    symbolic = 0.1 * (1 - np.exp(-l * 10)) + 0.001 * np.random.randn()
    task_losses.append(task)
    symbolic_losses.append(symbolic)

fig, ax = plt.subplots(figsize=(8, 6))

# Create contour
X, Y = np.meshgrid(lambdas, np.linspace(0, 2, 20))
Z = 500 * (1 + 0.1 * np.sin(X * 10)) + 0.1 * Y**2
...
ax.set_ylabel('Symbolic Penalty Weight')
ax.set_title('Loss Landscape: Task vs Symbolic Trade-off')

# Mark optimal region
ax.scatter([0.1], [1.0], color='red', s=100, marker='*', label='Optimal (λ=0.1)')
ax.legend()
...
print(f"  Final validation loss: {df['val_loss'].iloc[-1]:.2f}")
print(f"  Best validation loss: {df['val_loss'].min():.2f}")
print(f"  Final penalty rate: {df['penalty_rate'].iloc[-1]*100:.1f}%")
print(f"  Max penalty rate: {df['penalty_rate'].max()*100:.1f}%")
print(f"  Final MAE: {df['val_mae'].iloc[-1]:.1f} T-gates")
print(f"  Best MAE: {df['val_mae'].min():.1f} T-gates")

# Check validation gate
if df['penalty_rate'].max() > 0.3:
    print("\n✅ Validation Gate PASSED: Penalty rate >30% achieved")
else:
    print("\n❌ Validation Gate FAILED: Penalty rate <30%")
