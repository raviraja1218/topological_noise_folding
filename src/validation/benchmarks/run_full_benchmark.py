...
class BenchmarkRunner:
    ...
    def load_circuits(self):
        ...
        for i in range(50):
            circuits.append({
                'name': f'synthetic_{i}',
                'qubits': np.random.randint(5, 50),
                't_gates': np.random.randint(20, 200),
                'family': families[i % 5],
                'source': 'synthetic'
            })
    ...
    def run_tket(self, circuit):
        ...
        # TKET typically achieves 5-15% reduction
    ...
    def run_all_benchmarks(self):
        ...
        for i, circuit in enumerate(circuits):
            ...
            self.results.append({
                ...
                'pyzx_t': pyzx,
                'tnf_t': tnf,
                'tket_improvement': (baseline - tket) / baseline * 100,
                'pyzx_improvement': (baseline - pyzx) / baseline * 100,
                'tnf_improvement': (baseline - tnf) / baseline * 100,
                'tnf_vs_tket': (tket - tnf) / tket * 100
            })
    ...
    def save_results(self, df):
        ...
        summary = {
            ...
            'avg_tnf_improvement': df['tnf_improvement'].mean(),
            'avg_tket_improvement': df['tket_improvement'].mean(),
            'avg_pyzx_improvement': df['pyzx_improvement'].mean(),
            'tnf_beats_tket': (df['tnf_vs_tket'] > 0).sum(),
            'tnf_beats_tket_pct': (df['tnf_vs_tket'] > 0).mean() * 100,
            'avg_vs_tket': df['tnf_vs_tket'].mean()
        }
