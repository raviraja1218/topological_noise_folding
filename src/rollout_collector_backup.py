...
class RolloutCollector:
    ...
    def collect_rollouts(self, num_steps=2048):
        ...
        for worker in self.workers:
            result = self.pool.apply_async(worker.collect_rollout, args=(num_steps,))
...
class DistributedTrainer:
    ...
    def __init__(self, agent, env_creator, num_workers=12, buffer_size=100000):
        ...
        # Create policy function for workers
        def policy_fn(obs):
...
if __name__ == "__main__":
    ...
    # Create trainer
    trainer = DistributedTrainer(agent, env_creator, num_workers=4)
    
    # Test training step
    for i in range(3):
        stats = trainer.train_step(steps_per_update=100)
